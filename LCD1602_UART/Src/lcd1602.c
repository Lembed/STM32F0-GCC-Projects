...
{
    if (pData < 0) {
        DISP_PutChar(DISP_HandleStruct, '-');
        pData *= -1;
    } else {
        DISP_PutChar(DISP_HandleStruct, '+');
    }
    DISP_PutChar(DISP_HandleStruct, '0' + (pData / 1000000000)); //billion
    DISP_PutChar(DISP_HandleStruct, '0' + ((pData / 100000000) % 10)); //hundred million
    DISP_PutChar(DISP_HandleStruct, '0' + ((pData / 10000000) % 10)); //ten million
    DISP_PutChar(DISP_HandleStruct, '0' + ((pData / 1000000) % 10)); //million
    DISP_PutChar(DISP_HandleStruct, '0' + ((pData / 100000) % 10)); //hundred thousand
    DISP_PutChar(DISP_HandleStruct, '0' + ((pData / 10000) % 10)); //hundred thousand
    DISP_PutChar(DISP_HandleStruct, '0' + ((pData / 1000) % 10)); //thousand
    DISP_PutChar(DISP_HandleStruct, '0' + ((pData / 100) % 10)); //hundred
    DISP_PutChar(DISP_HandleStruct, '0' + ((pData / 10) % 10)); //ten
    DISP_PutChar(DISP_HandleStruct, '0' + (pData % 10)); //one
...
void DISP_PutString(DISP_HandleTypeDef *DISP_HandleStruct, char *data)
{
    while (*data != '\0') {
        DISP_PutChar(DISP_HandleStruct, *data++);
    }
}

void DISP_PutChar(DISP_HandleTypeDef *DISP_HandleStruct, char c)
{
